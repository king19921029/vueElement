...
<script>
  ...
  export default {
    ...
    created() {
      ...
      this.getrelationLifeList()
    },
    methods: {
      getrelationLifeList(params) {
        ...
        queryUserLifeHistoryList(postData).then(res => {
          this.listLoading = false;
          this.total = res.data.total;
          this.list = res.data.userLifeHistoryList;
          console.log(res)
        });
      },
      // 搜索
      ...
      handleShowProduct(index, row) {
        this.$router.push({
          path: '/userData/seeRelationLife',
          query: {
            userId: this.$route.query.id,
            id: row.user_life_history_id
          }
        });
      },
      //编辑
      handleUpdateProduct(idx, row) {
        this.$router.push({
          path: '/userData/updateRelationLife',
          query: {
            userId: this.$route.query.id,
            id: row.user_life_history_id
          }
      ...
      handleDelete(row) {
        this.$confirm('是否要进行删除操作?', '提示', {
          ...
          delUserLifeHistory({
            "userLifeHistoryId": row.user_life_history_id
          }).then(response => {
            this.$message({
              message: '删除成功',
              type: 'success',
