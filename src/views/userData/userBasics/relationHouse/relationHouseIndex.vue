  ...
  <div class="app-container">
    ...
    <div class="table-container">
      <el-table ref="productTable" v-loading="listLoading" :data="list" style="width: 100%" @selection-change="handleSelectionChange"
        ...
        <el-table-column label="房屋记录" width="140" align="center">
          <template slot-scope="scope">
            <el-button size="mini" @click="handleSeehouseRecord(scope.$index, scope.row)">
              管理房屋记录
            </el-button>
          </template>
        </el-table-column>
...
<script>
  ...
  // 房屋信息
  import {
    queryUserHouseRelationList,
    delUserHouseRelation
  ...
  export default {
    ...
    methods: {
      ...
      getHoseList(params) {
        ...
        console.log(params)
        queryUserHouseRelationList(params).then(res => {
          this.listLoading = false;
          this.total = res.data.total;
          this.list = res.data.userHouseRelationList;
          console.log(212, res)
        });
      },
      // 搜索
      ...
      handleShowProduct(index, row) {
        console.log("查看")
        this.$router.push({
          path: '/userData/glseeHouse',
          query: {
            id: row.user_house_relation_id
          }
        });
      },
      //编辑
      handleUpdateProduct(idx, row) {
        this.$router.push({
          path: '/userData/glupdateHouse',
          query: {
            userId: this.listQuery.userId,
            glId: row.user_house_relation_id
          }
        });
      },
      // 添加
      ...
      // 查看房屋记录
      handleSeehouseRecord(idx,row) {
        this.$router.push({
          path: '/userData/houseRecordIndex',
          query: {
            userId: this.listQuery.userId,
      ...
      handleDelete(row) {
        this.$confirm('是否要进行删除操作?', '提示', {
          ...
          delUserHouseRelation({
            "userHouseRelationId": row.user_house_relation_id
          }).then(response => {
            this.$message({
              message: '删除成功',
              type: 'success',
